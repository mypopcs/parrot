...
//判断点击或hover使用不同方式呈现
function doTipsProcess() {
    //获取DOM中所有标签名
    var elementArray = document.getElementsByTagName("pthighlight");
    for(var i = 0; i < elementArray.length; i++){
        /*addEventListener(event,fn,useCaption)
        * fn回调函数，useCaption用于描述是冒泡还是捕获。默认值是false，即冒泡传递。 当值为true，就是捕获传递
        */
        elementArray[i].addEventListener('mouseenter', hoverShow, false)
        elementArray[i].addEventListener('mouseout', hoverHide, false)
    }
}
